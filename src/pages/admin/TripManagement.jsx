...
const TripManagement = () => {
  ...
  }, [searchTerm, statusFilter, tripTypeFilter, pagination.current]);
  ...
  const openEdit = (trip) => {
    if (trip.status !== 'scheduled') {
      toast.error('Only scheduled trips can be edited');
      return;
    }

    setEditingTripId(trip._id);
    setSelectedCompany(trip.company?._id || '');

    // Populate form with trip data
    setFormData({
      tripName: trip.tripName || '',
      tripType: trip.tripType || 'login',
      routeName: trip.routeName || '',
      officeLocation: trip.officeLocation || { address: '', coordinates: { lat: 0, lng: 0 } },
      ...
      employees: trip.employees?.map(emp => ({
        ...
        pickupLocation: emp.pickupLocation || { address: '', coordinates: { lat: 0, lng: 0 } },
        dropLocation: emp.dropLocation || { address: '', coordinates: { lat: 0, lng: 0 } }
      })) || []
    });

    // Fetch employees for the company
    if (trip.company?._id) {
      fetchAvailableEmployees(trip.company._id);
    }
  ...
  const handleUpdateTrip = async () => {
    ...
    try {
      ...
      await tripAPI.update(editingTripId, payload);
  ...
  const renderTripModal = (isEdit) => {
    const isModalOpen = isEdit ? showEditModal : showCreateModal;
    const handleSubmit = isEdit ? handleUpdateTrip : handleCreateTrip;
    const modalTitle = isEdit ? 'Edit trip' : 'Create new trip';
    const submitButtonText = isEdit ? 'Update trip' : 'Create trip';
        ...
        <div className="flex items-center justify-center min-h-screen p-4">
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="px-6 py-4 space-y-6 text-sm">
              ...
              {/* Company - only show for create, not edit */}
              {!isEdit && (
                <div>
                  <label className="block text-[11px] font-medium mb-1 text-slate-700">
                    Company *
              ...
              <div>
                ...
                >
                  <input
                    ...
                    defaultValue={formData.officeLocation.address}
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500"
                  />
                </StandaloneSearchBox>
                {formData.officeLocation.address && (
                  ...
                  {formData.employees.map((item, index) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <button
                          onClick={() => removeEmployeeFromTrip(index)}
                          ...
                          <div>
                            ...
                            >
                              <input
                                ...
                                defaultValue={item.dropLocation.address}
                                className="w-full px-3 py-2 mt-1 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500"
                              />
                            </StandaloneSearchBox>
  ...
  return (
     <AdminLayout>
        <div className="min-h-[calc(100vh-64px)] bg-slate-50">
            ...
            {pagination.pages > 1 && (
              <div className="flex justify-center items-center gap-2 pt-4">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl text-sm font-medium text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Previous
                </button>
          ...
          {/* Create trip modal */}
          {renderTripModal(false)}

          {/* Edit trip modal */}
          {renderTripModal(true)}
        </div>
      </AdminLayout>
  );
};
